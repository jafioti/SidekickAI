...
class Dataset:
    ...
    Inputs:
        ...
        load_function (function) Parameters: (data (a dict with only the current index of data in it), global_index (the index to be loaded, global based on the start/end indexes passed into the dataset)): The function responsible for directly loading examples.  
    ...
    '''
    def __init__(self, batch_size, load_function, end_index, init_function=None, collate_function=None, start_index=0, preload=False, num_workers=0, **kwargs):
        ...
        self.data_chunk_size = min(min(max(int((self.end_index - self.start_index) / 5), 2000), 10000), self.end_index - self.start_index) # 10000: min, 50000: max | These are arbitrary
...
def load_job(data, other, start_index, end_index, batch_size, load_function, collate_function, batch_queue): # The job to be run in each worker
    ...
    for i in range(0, end_index - start_index - batch_size, batch_size):
        ...
        for item in batch: # Ensure all tensors are in shared memory to help with multiprocessing
