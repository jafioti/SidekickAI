...
class ComplexNorm(torch.nn.Module):
    ...
    def forward(self, complex_tensor: Tensor):
        r"""
        Args:
...
class TimeStretch(torch.nn.Module):
    ...
    def forward(self, complex_specgrams: Tensor, overriding_rate: Optional[float] = None):
        r"""
        Args:
...
class Fade(torch.nn.Module):
    ...
    def _fade_out(self, waveform_length: int):
...
class _AxisMasking(torch.nn.Module):
    ...
    def __init__(self, mask_param: int, axis: int, iid_masks: bool):
    ...
    def forward(self, specgram: Tensor, mask_value: float = 0.):
        r"""
        Args:
